...
# --- 3. تهيئة محرك RAG داخل Session State وتحميل Client الموديل ---
if "rag_engine" not in st.session_state:
    st.session_state.rag_engine = SimpleRAGEngine()
...
@st.cache_resource
def load_llm_client():
    # جلب التوكن من st.secrets أو من متغيرات البيئة
    hf_token = st.secrets.get("HF_TOKEN", os.getenv("HF_TOKEN"))
    # استدعاء الموديل عبر Hugging Face Serverless API
...
if prompt := st.chat_input("اطرح سؤالك حول الملف المرفوع..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("جاري البحث والتوليد..."):
            ...
            if context:
                system_instruction = (
                    "أنت مستشار قانوني متخصص في المناقصات. أجب على سؤال المستخدم بناءً على النصوص المستخرجة من الكراسة أدناه فقط.\n"
                    "إذا لم تجد الإجابة في النص المرفق، أبلغ المستخدم بوضوح أن المعلومة غير موجودة في الكراسة المرفوعة.\n\n"
                    f"نصوص الكراسة المرفقة:\n{context}"
                )
            else:
            ...
            # إضافة آخر رسالتين من السجل للسياق
            ...
            # التوليد عبر API بطريقة Streaming
